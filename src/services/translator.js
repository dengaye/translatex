...
class TranslatorService {
   ...
   * @param {string} from 源语言，auto为自动检测 
   * @param {string} to 目标语言
   * @returns {Promise<Object>} 翻译结果
   */
  static async youdaoTranslate(text, from = 'auto', to = 'zh-CHS') {
    try {
      // 有道智云API基本参数
   ...
   * 生成有道API签名
   */
  static generateSign(appId, text, salt, curtime, appSecret) {
   ...
   * @returns {Promise<Object>} 翻译结果
   */
  static async deeplTranslate(text, targetLang = 'ZH') {
    try {
      // DeepL API基本参数
   ...
   * 翻译文本，自动选择合适的翻译API
   * @param {string} text 要翻译的文本
   * @param {string} api 要使用的API，默认为有道
   * @returns {Promise<Object>} 翻译结果
   */
  static async translate(text, api = 'youdao') {
    if (!text || text.trim() === '') {
      return {
        success: false,
        error: '翻译文本不能为空',
